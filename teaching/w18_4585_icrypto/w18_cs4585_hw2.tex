...
\begin{questions}
  ...
  preserving pseudorandom function on $n$-bit inputs. In each of the
  following cases, decide whether $G'$ is necessarily a pseudorandom
  generator. If yes, give a proof; if not, show a counterexample.
    
  \begin{parts}
    \part[6] $G'(s) := G(s_1,\ldots, s_{\lfloor n/2 \rfloor})$, where
    $s = s_1,\ldots, s_n$.
    \part[6] $G'(s) := G(0^{|s|} \| s)$.
  ...
  \begin{parts}
    ...
    function $F': \bit^n \times \bit^{n-1} \times \bit^{2n}$, decide
    if $F'$ is a pseudorandom function. If yes, prove it; if not, show
    an attack.
