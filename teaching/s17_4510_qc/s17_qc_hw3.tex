...
\begin{questions}
  ...
  $M$:
    ...
    \end{equation*}
    where $\hat \alpha_y = \frac{1}{\sqrt M}\sum_{x} \omega_M^{xy}$,
    $\omega_N = e^{2\pi i /M}$, for each $y \in [M]$.
...
\begin{parts}
  ...
  $\cos(\theta_t)\ket{A} + \sin(\theta_t)\ket{B}$. How does $\theta_t$
  change to $\theta_{t+1}$? (Hint: geometric interpretation of
  Grover's algorithm)
